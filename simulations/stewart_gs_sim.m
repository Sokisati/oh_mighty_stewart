%% stewart_gs_sim.m
%  Stewart Platform - Gain Scheduling (Non-linear PID) Simulation
%
...
T_sim_gs = 30;
disturb_table = generate_disturbances(seed, T_sim_gs);
noise_table = generate_sensor_noise(seed, T_sim_gs, dt);

% Temporarily override N and t_vec for 30 seconds
N = round(T_sim_gs / dt);
t_vec = linspace(0, T_sim_gs, N)';
pos_ref_gs = zeros(N, 6);
pos_ref_gs(:, 3) = h0;   % fixed height
...
Kp_0 = 15;
Kd_0 = 1.56;
Ki_0 = 0.1; % Integral effect (set > 0 for steady realistic wind)

% Gain Scaling Factors (Increase per meter of error)
alpha_p = 250.0; 
alpha_i = 10.0;  % Gain for integral scaling
alpha_d = 60.0;  
...
%% =========================================================
fprintf('Running GS closed-loop simulation...\n');

sim_params = struct();
sim_params.dt         = dt;
sim_params.T_sim      = T_sim_gs;
...
sim_params.gs_alpha_p = alpha_p;
sim_params.gs_alpha_i = alpha_i;
sim_params.gs_alpha_d = alpha_d;
sim_params.gs_beta_i  = 15.0;

sim_result = simulate_ball(Kp_0, Ki_0, Kd_0, sim_params, disturb_table, noise_table);

% Unpack results into workspace variables expected by animation code
ball_x_gs  = sim_result.ball_x;
ball_y_gs  = sim_result.ball_y;
ball_vx_gs = sim_result.ball_vx;
ball_vy_gs = sim_result.ball_vy;
...
pos_ref_gs = zeros(N, 6);
pos_ref_gs(:, 3) = h0;
pos_ref_gs(2:end, 4) = roll_log(1:end-1);
pos_ref_gs(2:end, 5) = pitch_log(1:end-1);
...
ball_world_gs  = zeros(N, 3);
ball_on_plate_gs = true(N, 1);
R_all_gs       = zeros(3,3,N);
top_ring_gs    = zeros(n_ring,3,N);
P_top_gs       = zeros(6,3,N);
leg_x_gs       = nan(18,N);
leg_y_gs       = nan(18,N);
leg_z_gs       = nan(18,N);

fall_start_gs  = N;
fall_pos_gs    = zeros(3,1);
fall_vel_gs    = zeros(3,1);

for i = 1:N
    rx_i=pos_ref_gs(i,4); ry_i=pos_ref_gs(i,5); rz_i=pos_ref_gs(i,6);
    ...
    top_ring_gs(:,:,i) = (ring_world + [0;0;h0])';
    for k = 1:6
        P_top_gs(k,:,i) = (R_i*P_top_b(k,:)' + [0;0;h0])';
        idx = (k-1)*3 + 1;
        leg_x_gs(idx,  i) = P_base(k,1);  leg_x_gs(idx+1,i) = P_top_gs(k,1,i);
        leg_y_gs(idx,  i) = P_base(k,2);  leg_y_gs(idx+1,i) = P_top_gs(k,2,i);
        leg_z_gs(idx,  i) = P_base(k,3);  leg_z_gs(idx+1,i) = P_top_gs(k,3,i);
    end

    % Ball world position (with fall-off check)
    if ball_on_plate_gs(i)
        bw = R_i*[ball_x_gs(i);ball_y_gs(i);r_ball] + [0;0;h0];
        ball_world_gs(i,:) = bw';

        % Check plate boundary
        if norm([ball_x_gs(i), ball_y_gs(i)]) > r_limit && i < N
            ball_on_plate_gs(i+1:end) = false;
            fall_start_gs = i + 1;
            fall_pos_gs   = bw;
            fall_vel_gs   = R_i * [ball_vx_gs(i); ball_vy_gs(i); 0];
            fprintf('  Ball left plate (PID) at t = %.2f s\n', t_vec(i));
        end
    else
        % Free fall in world frame
        t_fall = t_vec(i) - t_vec(fall_start_gs);
        bw = fall_pos_gs + fall_vel_gs*t_fall + [0;0;-0.5*g_acc*t_fall^2];

        if bw(3) < r_ball   % hit floor
            bw(3) = r_ball;
            ball_world_gs(i:end,:) = repmat(bw', N-i+1, 1);
            break;
        end
        ball_world_gs(i,:) = bw';
    end
...
settled = find(sqrt(ball_x_gs.^2 + ball_y_gs.^2) < 0.005 & t_vec > 0.5, 1);
if ~isempty(settled)
    fprintf('%.2f s\n\n', t_vec(settled));
else
...
title(ax,'Gain Scheduling Control - Stewart Platform', ...
...
h_top_pts = plot3(ax,P_top_gs(:,1,1),P_top_gs(:,2,1),P_top_gs(:,3,1), ...
...
for k = 1:6
    idx = (k-1)*3 + 1;
    h_legs(k) = plot3(ax, leg_x_gs(idx:idx+1,1), leg_y_gs(idx:idx+1,1), leg_z_gs(idx:idx+1,1), ...
...
kp_txt = text(ax, lim*0.5, lim*0.8, h0*2.2, 'Kp: 10.0', 'Color',[0.3 1.0 0.4],'FontSize',11,'FontWeight','bold');
ki_txt = text(ax, lim*0.5, lim*0.8, h0*2.0, 'Ki: 0.0', 'Color',[0.3 0.8 1.0],'FontSize',11,'FontWeight','bold');
kd_txt = text(ax, lim*0.5, lim*0.8, h0*1.8, 'Kd: 1.56', 'Color',[1.0 0.8 0.2],'FontSize',11,'FontWeight','bold');

% Trail
trail_len = 15;
trail_buf = repmat(ball_world_gs(1,:), trail_len, 1);
...
for i = 1:N
    ...
    set(h_top_pts, 'XData',P_top_gs(:,1,i),'YData',P_top_gs(:,2,i),'ZData',P_top_gs(:,3,i));
    for k = 1:6
        idx = (k-1)*3 + 1;
        set(h_legs(k), 'XData',leg_x_gs(idx:idx+1,i), 'YData',leg_y_gs(idx:idx+1,i), 'ZData',leg_z_gs(idx:idx+1,i));
    end
    ...
    if Kp_log(i) > Kp_0 * 1.5
        set(kp_txt, 'Color', [1.0 0.3 0.2]);
    else
        set(kp_txt, 'Color', [0.3 1.0 0.4]);
    end
    trail_buf(1:end-1,:) = trail_buf(2:end,:);
    trail_buf(end,:) = bw;
    tc = [0.3, 0.9, 1.0, 0.7];                      % cyan = on plate
    if ~ball_on_plate_gs(i), tc = [1.0,0.15,0.15,0.9]; end  % red = free fall
...
%  RESULTS PLOTS
%% =========================================================
figure('Name','Gain Scheduling Results','Color',[0.10 0.10 0.13],'NumberTitle','off', ...
...
hold on; grid on;
plot(t_vec, ball_x_gs*100, 'Color',[0.3 0.6 1.0],'LineWidth',1.8,'DisplayName','X');
plot(t_vec, ball_y_gs*100, 'Color',[1.0 0.4 0.3],'LineWidth',1.8,'DisplayName','Y');
...
% Scheduled Gains Plot
subplot(4,1,2);
set(gca,'Color',[0.08 0.08 0.10],'XColor',[0.8 0.8 0.8],'YColor',[0.8 0.8 0.8],'GridColor',[0.4 0.4 0.4]);
hold on; grid on;
plot(t_vec(1:end-1), Kp_log(1:end-1), 'Color',[0.3 1.0 0.4],'LineWidth',1.8,'DisplayName','Kp (Scheduled)');
plot(t_vec(1:end-1), Ki_log(1:end-1), 'Color',[0.3 0.8 1.0],'LineWidth',1.8,'DisplayName','Ki (Scheduled)');
plot(t_vec(1:end-1), Kd_log(1:end-1), 'Color',[1.0 0.8 0.2],'LineWidth',1.8,'DisplayName','Kd (Scheduled)');
yline(2.28,'--','Color',[0.5 0.5 0.5],'LineWidth',1,'Label','Kp_0');
yline(1.14,'--','Color',[0.5 0.5 0.5],'LineWidth',1,'Label','Kd_0');
ylabel('Gains','Color',[0.8 0.8 0.8]);
title('Real-time Scheduled Parameters','Color',[0.95 0.95 0.95],'FontWeight','bold');
legend('Location','best','TextColor',[0.8 0.8 0.8],'Color',[0.12 0.12 0.15]);
...
set(gca,'Color',[0.08 0.08 0.10],'XColor',[0.8 0.8 0.8],'YColor',[0.8 0.8 0.8],'GridColor',[0.4 0.4 0.4]);
hold on; grid on;
dist_gs = sqrt(ball_x_gs.^2 + ball_y_gs.^2) * 100;
plot(t_vec, dist_gs,'Color',[0.9 0.5 1.0],'LineWidth',2.0);
...
%% =========================================================
fprintf('\n================================================\n');
fprintf('         GAIN SCHEDULING METRICS\n');
fprintf('================================================\n');

d0 = dist_gs(1);

% 1. Rise Time (90% to 10% of initial distance)
idx_90 = find(dist_gs <= 0.9*d0, 1, 'first');
idx_10 = find(dist_gs <= 0.1*d0, 1, 'first');
...
is_settled = dist_gs(1:idx_first_dist) < settle_thresh;
...
if ~isempty(idx_10) && idx_10 < idx_first_dist
    max_overshoot = max(dist_gs(idx_10:idx_first_dist));
...
for d = 1:size(disturb_table,1)
    ...
    window_dist = dist_gs(idx_d:idx_next);
...
if ~isempty(idx_last_2s)
    sse = rms(dist_gs(idx_last_2s:end));
